...
/**
 * REST controller for managing {@link tn.soretras.contestmanager.domain.Contestform}.
 */
@RestController
@RequestMapping("/api")
public class ContestformResource {
    ...
    public ResponseEntity<ContestformDTO> updateContestform(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, contestformDTO.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<ContestformDTO> partialUpdateContestform(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, contestformDTO.getId())
        );
    }
    ...
    public ResponseEntity<List<ContestformDTO>> getAllContestforms(
        @org.springdoc.api.annotations.ParameterObject Pageable pageable,
        @RequestParam(required = false, defaultValue = "false") boolean eagerload
    ) {
        log.debug("REST request to get a page of Contestforms");
        Page<ContestformDTO> page;
        if (eagerload) {
            page = contestformService.findAllWithEagerRelationships(pageable);
        } else {
            page = contestformService.findAll(pageable);
        }
    ...
    public ResponseEntity<ContestformDTO> getContestform(@PathVariable String id) {
    ...
    public ResponseEntity<Void> deleteContestform(@PathVariable String id) {
